...
namespace IC2.Controllers
{
    public class CargarDatosRoamingController : Controller
    {
        ...
        public object CargaTAPINA(string periodo, string ruta, string nombre, int Id, char separador)
        {
            ...
            try {
                lineas = System.IO.File.ReadAllLines(ruta + "/" + nombre).Where(x => !string.IsNullOrWhiteSpace(x)).ToArray();
                ...
                try {
                    ...
                    using (TransactionScope scope = new TransactionScope()) {
                        for (i = 1; i < lineas.Count(); i++) {
                            ...
                            list.Add(datosROM);
                            Log log2 = new Log();
                            log2.insertaNuevoOEliminado(datosROM, "Nuevo", "datosTraficoTAPINA.html", Request.UserHostAddress);
                ...
                try {
                    ...
                    using (TransactionScope scope = new TransactionScope()) {
                        for (i = 1; i < lineas.Count(); i++) {
                            ...
                            list.Add(datosROM);
                            Log log2 = new Log();
                            log2.insertaNuevoOEliminado(datosROM, "Nuevo", "datosTraficoTAPINB.html", Request.UserHostAddress);
                ...
                try {
                    ...
                    using (TransactionScope scope = new TransactionScope()) {
                        for (i = 1; i < lineas.Count(); i++) {
                            ...
                            Log log6 = new Log();
                            log6.insertaNuevoOEliminado(datosROM, "Nuevo", "datosTraficoTAPOUTA.html", Request.UserHostAddress);
                ...
                try {
                    ...
                    using (TransactionScope scope = new TransactionScope()) {
                        for (i = 1; i < lineas.Count(); i++) {
                            ...
                            Log log2 = new Log();
                            log2.insertaNuevoOEliminado(datosROM, "Nuevo", "datosTraficoTAPOUTB.html", Request.UserHostAddress);
